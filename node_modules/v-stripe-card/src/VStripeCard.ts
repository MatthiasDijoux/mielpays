...
export default base.extend<options>().extend({
  ...
  methods: {
    ...
    checkAPIKey (key: string): void {
      ...
      if (/^sk_/.test(key)) {
        console.error(
          '[VStripeCard Error] It looks like you are using a SECRET API key! You should be using your PUBLIC API key. If you have mistakenly made your secret key publicly available on the web, you should strongly consider rolling your API keys. See: https://stripe.com/docs/keys#keeping-your-keys-safe'
        )
      }
      // if they are using a test key
      if (/^pk_test/.test(key)) {
        console.warn(
          '[VStripeCard Warning] Your `v-stripe-card` instance was initialized with a TEST key. This is correct and appropriate for development and/or testing environments. For production environments, please use your LIVE key. See https://stripe.com/docs/keys for more information.'
        )
      }
    },
    clearableCallback () {
    ...
    genInput (): VNode {
      return this.$createElement('div', {
        attrs: { id: this.computedId, tabindex: -1 },
      })
    },
    /**
      ...
      if (typeof Stripe === 'function') {
        ...
        if (document.querySelector(`script[src="${src}"]`)) {
          ...
          if (typeof this.$loadScript === 'undefined') {
            ...
            throw new Error(
              '[VStripeCard Error]: Stripe is not available and could not be loaded. Please make sure that you have installed and configured all of the necessary dependencies to use this component.'
            )
          } else {
            ...
            this.$loadScript(src)
              ...
              .catch((error: Error) => {
                ...
                throw new Error(
                  '[VStripeCard Error] There was a problem loading Stripe: ' +
                    error.message
                )
              })
          }
